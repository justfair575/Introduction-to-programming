...
class ColorImage {
	...
	ColorImage blackWhite(){
		...
		for(int x=0; x!=getWidth();x++)
			for(int y=0; y!=getHeight();y++)
				img.setColor(x,y,getColor(x,y).grayEquivalent());
	...
	void light(int n){
		for(int x=0; x!=getWidth();x++)
			for(int y=0; y!=getHeight();y++)
				setColor(x,y,getColor(x,y).brighter(n));
	 ...
	 void mirror(){
		 ...
		 for(int x = 0; x!=half;x++)
			 for(int y=0; y!=getHeight();y++){
				 Color aux = getColor(x,y);
		 setColor(x,y,getColor((getWidth()-1)-x,y));
		 setColor((getWidth()-1)-x,y,aux);
			 }
	 }
	...
	void invert(){
		for(int x=0; x!=getWidth();x++)
			for(int y=0; y!=getHeight();y++)
				setColor(x,y,getColor(x,y).inverse());
	}
